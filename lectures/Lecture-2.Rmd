...
`while` loops are not necessarily as common as `for` loops introduced above, but they are quite important. 
...
What is the value of `foo` right now!

**Important concept:** Note that I used the variable `foo` on both sides of the `<-` operator above. 

We can also use indexs to loop through vectors (and other objects).
...
For the remainder of class, we will work on [Problem Set 2](http://mlammens.github.io/ENS-623-Research-Stats/problem_sets/Problem-set-2.html)
